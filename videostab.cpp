...
void run()
{
    ...
    {
#ifdef TimeDebug
		cout << "global:"<<((double)getTickCount()-t)/getTickFrequency()<<endl;
#endif
        nframes++;
		...
		imshow("stabilized", stabilizedFrame);
		char c;
		if ((c = waitKey(20)) == 27)
		{
			break;
	...
	cout << "Stablization finished, totally processed " << nframes << "frames" << endl;
...
void printHelp()
{
    cout << "OpenCV video stabilizer.\n"
    		...
    		"  -zm, --zero-motions=(no|<int_number>)\n"
    		"      Filter out all motions starting from a specific frame. The default is no.\n"
            "  -q, --quiet\n"
...
public:
    ...
    virtual Ptr<ImageMotionEstimatorBase> build()
    {
		...
		if(gpu)
		{
			KeypointBasedMotionEstimatorOcl * kbest = new KeypointBasedMotionEstimatorOcl(est);
			kbest->detector_.maxCorners = 100;
...
int main(int argc,const char *argv[])
{
	try
	{
...
#ifdef VideoSource
		cout << "Please input the camera number\n";
		int num =0;
		cin >> num;
		VideoCaptureSource * source = new VideoCaptureSource(num);
#else
		...
		{
			stabilizer->setStartingFrame(20);
			stabilizer->setRadius(100);
		}
...
//int listenclient(SOCKET &server, SOCKET &client)
//{
//	int retVal;
...
//		nReadLen = recv(client, camera, 1, 0);
//		run(camera[0], argc, argv, server, client);
//
//		
//	}
//}
